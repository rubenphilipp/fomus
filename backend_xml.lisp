                  ...
                  ,.(loop with slrlvl = (cons nil nil) and wlvl = (cons nil nil) and olvl = (cons nil nil) and tlvl = (cons nil nil)
                     ...
                     `("measure" ("number" ,mn)
                       ,@(let ((ss (getprop m :startsig))
                               ...
                               (cc (getprops m :clef))
                               (bar-mode :metered))
                           (let ((tmp-md (second (getprop (meas-timesig m) :mode))))
                             (cond ((eq tmp-md :metered)
                                    (setf bar-mode :metered))
                                   ((eq tmp-md :unmetered)
                                    (setf bar-mode :unmetered))
                                   (t nil)))
                           (when (or (= mn 1) ss cc)
                             `(("attributes" nil
                                             ...
                                             ,@(when kk (let ((x (lookup (second (getprop (meas-timesig m) :keysig)) +xml-keysigs+)))
                                                          (when x `(("key" nil
                                                                           ("fifths" nil (car x))
                                                                           ("mode" nil (cdr x)))))))
                                             ,@(when (and ss (eq :metered bar-mode)) `(("time" ,(when (eq *timesig-style* :common)
